...
public class MergeTest {
    ...
    public void mergePublishersSimpleTest() {
        Flux.merge(singleDigitIntProducer(), doubleDigitIntProducer(), tripleDigitIntProducer()) // the order of placing publishers does not matter as all the publishers are subscribed at the same time.
    ...
    private static Flux<Integer> tripleDigitIntProducer() {
    ...
    public void flux_publisher_merge_with_delayElements_test() {
        ...
        // This is a useful test to actually understand the behavior of merge.
        StepVerifier.create(mergedFlux.log())
                .expectSubscription()
                .expectNext("Adam")
                .expectNext("A")
    ...
    @Test
    public void flux_publisher_mergeSequential_test() {
        Flux<String> alphabetFlux = Flux.just("A", "B", "C")
    ...
    /* *
     * mergeDelayError(): The mergeDelayError() merges data from Publisher sequences contained in an array into an interleaved merged sequence.
     * This is similar to concatDelayError(), only publishers are subscribed eagerly.
